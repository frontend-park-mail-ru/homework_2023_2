...
QUnit.module("Тестируем функцию euclid", function () {
	...
	QUnit.test(
		"Функция должна правильно находить НОД трёх натуральных чисел",
		function (assert) {
	...
	QUnit.test(
		"Функция должна правильно работать с любым количеством аргументов",
		function (assert) {
			...
			assert.strictEqual(
				euclid(...[...temp, ...temp, ...temp, ...temp, ...temp]),
				euclid(...temp)
			);
		}
	);
